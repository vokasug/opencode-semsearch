...
- Prune в `lazy_index()`: при каждом вызове из индекса удаляются вектора сообщений/сессий, которых больше нет в `opencode.db` — поиск больше не находит удалённые сессии. Счётчик `pruned_orphans` в timings
